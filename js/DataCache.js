...
function DataCache(name,isSaveFile){
     ...
     * @author:wangkun
     * @date:2017-04-10
     * @param:elements-要素(数组)；hourspans-时次(数组)
     * @return:
     * @description:从内存中获取数据
     */
    this.initCachesFromMemory=function(elements,hourspans){
    	var elementSize=elements.length;
    	var hourspanSize=hourspans.length;
    	var maketime = GDYB.GridProductClass.currentMakeTime;
            var datetime = GDYB.GridProductClass.currentDateTime;
	var version = GDYB.GridProductClass.currentVersion
	var filePath = 'Documents/' + t.name + '/'+maketime+'/'+datetime+'/'+version+'/';
    	for(var i=0;i<elementSize;i++){
    		var element=elements[i];
    		for(var j=0;j<hourspanSize;j++){
    			var hourspan=hourspans[j];
    			getData1(element,hourspan);
    		}
    	}
                    ...
                    dirEntry.getFile('data1.txt', {}, function (fileEntry) {
                        fileEntry.file(function(file){
                            ...
                            reader.onloadend = function (e)
                            {
                                ...
                                if(!maketimeData.hasOwnProperty(datetime))
                                    maketimeData[datetime] = {};
                                var dateData = maketimeData[datetime];
