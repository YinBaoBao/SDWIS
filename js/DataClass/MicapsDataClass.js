...
function MicapsDataClass() {
    ...
    this.displayMicapsData=function(recall, element, level, datetime, hourspan,curMap){
...
//        map.setCenter(new WeatherMap.LonLat(110, 35), 3); //设置可视范围
        if(element!=null && element!=""){
            ...
            if(!(this.currentElement == "surface_plot" || this.currentElement == "high_plot" || this.currentElement == "fy2_ir1" || this.currentElement == "fy2_ir2" || this.currentElement == "fy2_ir3" || this.currentElement == "fy2_ir4" || this.currentElement == "fy2_vis")) {
    ...
    this.addLayer = function(pMap){
        ...
        if(this.currentElement == "surface_plot" || this.currentElement == "surface")
        {
            ...
            this.layerPlot.renderer.plotWidth = 60;
            this.layerPlot.renderer.plotHeight = 50;
        }
        else if(this.currentElement == "high_plot" || this.currentElement == "high")
        {
            this.layerPlot.renderer.styles = plotStyles_hight;
            this.layerPlot.renderer.plotWidth = 50;
            this.layerPlot.renderer.plotHeight = 40;
        }
                        ...
                        if (typeof(data) != "undefined") {
                            ...
                            else if (data.hasOwnProperty("dvalues")) //格点数据
                            {
                                ...
                                if (dvalues != null && dvalues.length > 0) {
                                    var dMin = 9999;
                                    var dMax = -9999;
                                    datasetGrid = new WeatherMap.DatasetGrid(data.left, data.top, data.right, data.bottom, data.rows, data.cols, 1);
    ...
    this.removeFeatures = function () {
        ...
        if(this.layerFillRangeColor!=null){
            this.layerFillRangeColor.removeAllFeatures();
        }
    }
}