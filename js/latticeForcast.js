...
function initChartTable(){
    if($("#latticeForcast").html() == ""){
        var contentForcast = '<div>'+
            ...
            '<canvas id="temperature" height="100" width="1100" style="margin-top: 5px;"></canvas>'+
            ...
            '<canvas id="precipitation" height="100" width="1100" style="margin-top: 5px;"></canvas>'+
            ...
            '<canvas id="windSpeed" height="100" width="1100" style="margin-top: 5px;"></canvas>'+
            ...
            '<canvas id="rh" height="100" width="1100" style="margin-top: 5px;"></canvas>'+
            ...
            '<canvas id="tcc" height="100" width="1100" style="margin-top: 5px;"></canvas>'+
            ...
            '<canvas id="vis" height="100" width="1100" style="margin-top: 5px;"></canvas>'+
...
function updateChartTable(items, dateTime){
    ...
    updateChart("precipitation", "Bar", items, dateTime, "r3");
	...
	updateChartCP("forcastForm", items, dateTime, ["2t","r3"]);
...
function updateWeatherDescription(items, hourSpan3, hourSpan12, hourSpan24){
    ...
    var tcc = getValueFromForecastData(items, "tcc", hourSpan3);
    var vis = getValueFromForecastData(items, "vis", hourSpan3)!=null?getValueFromForecastData(items, "vis", hourSpan3):"空";
    ...
    var result = "未来3小时气温"+temp+"℃，降水"+r3+"mm，"+wd3+ws3+"，相对湿度"+rh+"%，云量"+tcc+"%，能见度"+vis+"公里；<br>"+
        "未来24小时"+(w12==w24?w12:(w12+"转"+w24))+"，降水"+r24+"mm，"+wd+ws+"，气温"+tmin+"-"+tmax+"℃，空气污染气象条件等级为"+air+"级。"
    $("#weatherDescription").html(result);
}
...
function updateChart(chartId, chartType, items, dateTime, element){
    ...
    var chartDataWd = {labels:[], data:[]};
    if(element == "ws3")
        chartDataWd = getChartDataFromForecastData(items, "wd3", dateTime);
    for(var i=0;i<chartDataWd.data.length;i++){
        chartDataWd.data[i] = getWDValue(chartDataWd.data[i]);
    }
    if(chartData != null){
        var lineChartData = {
        ...
        if(element == "ws3")
            new Chart(document.getElementById(chartId).getContext("2d")).Line(lineChartData, {pointDot : false, isWind : true});
...
//更新产品展示统计图
function updateChartCP(chartId, items, dateTime, elements){
	var chartData = getChartDataFromForecastDataCP(items, elements, dateTime);
	if(chartData != null){
		var lineChartData = {
			labels : chartData.labels,
...
function updateChartTime(dateTime, hourSpans){
    ...
    var widthTotal = parseInt($(".latticeTime").css("width"));
...
function updateTable(items, dateTime){
    ...
    var hourSpanTotal = 72; //最长十天，这里写死基本上没问题
    ...
    var contentHtml = "";
    contentHtml += '<table class="latticeTable"><tr class="tableHeader"><td rowspan="2" style="width: 80px">预报要素</td>';
    ...
    contentHtml += '</tr><tr><td class="tableHeader">3小时降水</td>';
    for(var i=0;i<days;i++){
...
//            }
    ...
    contentHtml += '</tr><tr><td class="tableHeader">6小时降水</td>';
    for(var i=0;i<days;i++){
    ...
    contentHtml += '</tr><tr><td class="tableHeader">12小时降水</td>';
    for(var i=0;i<days;i++){
    ...
    contentHtml += '</tr><tr><td class="tableHeader">24小时降水</td>';
    for(var i=0;i<days;i++){
...
function getChartDataFromForecastData(items, element, dateTime){
    ...
    for(var key in items){
        ...
        if(item.element == element){
            var d = item.datas[0];
            if(item.datas[0] == -9999)
                d = "";
            if(element == "ws3" && (d == 0 || d == ""))
                d = 0.00000001;
            datas.push(d);
            if(key%2==1) //间隔1个显示时间
                labels.push("");
            else
            {
                ...
                labels.push((Array(2).join(0) + hour).slice(-2) + ":" + (Array(2).join(0) + minutes).slice(-2));
            }
        }
    }
    if(datas.length > 0)
...
//从预报数据中获取图表数据(产品展示)
function getChartDataFromForecastDataCP(items, elements, dateTime){
	var time = dateTime.getTime();
	var chartData = null;
	var labels1 = [];
	var labels2 = [];
