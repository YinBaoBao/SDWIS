...
var AlertSignalProduce = clazz({
	CONST: { //常量
		...
		icon_url: T.CTX + 'imgs/warning/', // 图标地址
	...
	getAlertIcon: function(icon) {
		return this.CONST.icon_url + icon + '.png';
	},
	...
	stepPage1: function($mainPage, $footer, index) {
		...
		// 发布单位 用户部门
		$formSec_info.find('select[name="city"]').html(`<option data-code="${user_areaCode}" value="${user_areaName}">${user_areaName}气象局</option>`);
	...
	applyTree: function() {
		...
		var me = this,
			...
			i, j;
		// 市级用户
		if (this.option.user.areaCode != 14) {
			var areaCode = this.option.user.areaCode;
			for (i = 0; i < loopI; i++) {
				var city = cities[i];
				if (areaCode === city.code) {
					cities = [city];
					loopI = cities.length;
					break;
				}
		...
		for (i = 0; i < loopI; i++) {
			...
			var cityIndex = tree.length - 1;

			// 遍历县
			for (j = 0; j < loopJ; j++) {
				var country = countries[j];
				var checked = expr && new RegExp(expr).test(country.code);
				if (checked) { //展开父节点
					tree[cityIndex].open = true;
					tree[cityIndex].checked = true;
				}
