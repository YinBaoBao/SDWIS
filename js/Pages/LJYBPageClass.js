...
function LJYBPageClass() {
	...
	this.renderMenu = function () {
		 ...
		 */
		function InitRes() {
			t.MyTime = new DateSelecter(1, 1, "", true); //最小视图为分钟
			t.MyTime.intervalMinutes = 6; //6分钟
			var date = new Date();
			var moreMin = date.getMinutes() % 6;
			t.MyTime.changeHours(-moreMin);
	...
	this.submitProduct=function(){
		...
		var filename="lj"+t.filedatetime+".doc";
		var imgContent=src.replace("data:image/png;base64,","");
		var forcastor=$("#forcastor").find("option:selected").text();
		var departname=$.cookie("showName");
		fnCn.submitDLProduct(filename,forcastor,departname,t.forcasttime,content,imgContent,function(){
			layer.msg("生成成功!");
			$("#creatingProductWindow").css("display", "none");
		},function(){
			layer.msg("生成失败!");
...
LJYBPageClass.prototype = new PageBase();
