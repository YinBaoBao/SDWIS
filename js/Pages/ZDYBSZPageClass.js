...
function ZDYBSZPageClass(){
    ...
    function initZDYBSetPage(obj){
        ...
        if($.inArray("qutai", departList)!=-1){
            contentHtml += "<div class='zdybSetArea zdybSetActive'style='background-color: rgb(158, 195, 255);' flag='qutai'>省台</div>";
        }
        else{
            contentHtml += "<div class='zdybSetArea' flag='qutai'>省台</div>";
            display = "none";
        }
        ...
        contentHtml +="<span style='margin-left: 14px;'>截止时间：</span><select class='zdybSetPublishTime'>";
        for(var i=0;i<24;i++){
        ...
        contentHtml +="</select><span>分</span>" +
            "<span style='margin-left: 14px;'>格点来源：</span><select class='zdybSetPublishTime' style='padding: 2px;'>";
        if(obj.id==-1){
            contentHtml += "<option selected='true' value='prvn'>省台</option>";
            ...
            contentHtml += "<option value='prvn'>省台</option>";
        ...
        $.ajax({
            ...
            success: function (data) {
                var list = JSON.parse(data);
                var contentHtml = "<span style='margin-left: 14px;'>格点预报时次：</span><select class='zdybSetPublishTime'>";
                for(var i=0;i<list.length;i++){
                ...
                contentHtml += "</select>";
                if(obj.postType == "r")
                    contentHtml += "<span style='margin-left: 14px;'>岗位：</span><select class='zdybSetPublishTime' style='width: 70px;'><option selected='true' value='r'>领班岗</option><option value='p'>首席岗</option>";
               else
                    contentHtml += "<span style='margin-left: 14px;'>岗位：</span><select class='zdybSetPublishTime' style='width: 70px;'><option value='r'>领班岗</option><option selected='true' value='p'>首席岗</option>";
    ...
    function zdybSetSave(obj){
        ...
        for(var i=0;i<list.length;i++){
            if($(list[i]).hasClass("zdybSetActive")){
                ...
                if($(list[i]).attr("flag") == "qutai")
                    postType = $(list[i]).parent().find("select")[6].value;
            }
            else{
                gdybPublishTime += "-1,";
                endTime += "-1,";
        ...
        if(obj.id!=-1){
            var param = '{"id":'+obj.id+',"publishTime":"'+publishTime+'","gdybPublishTime":"'+gdybPublishTime+'","makeTime":"'+makeTime+'","forecastTime":"'+forecastTime+'","gdybType":"'+gdybType+'","endTime":"'+endTime+'","stationNums":"'+stationNums+'","depart":"'+depart+'","areaCodes":"'+areaCodeStr+'","outType":"'+outType+'","hourSpan":"'+hourSpan+'","hourSpanTotal":"'+hourSpanTotal+'","postType":"'+postType+'","ui":"'+ui+'"}';
            $.ajax({
                type: 'post',
