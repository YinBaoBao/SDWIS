...
*
* */
function DateSelecter(minView,startView,format,addMinute){
    this.div = null;
    this.leftBtn = null;
    this.rightBtn = null;
    this.input = null;
    ...
    this.createInput = function(){
        ...
        this.input = $("<input>")
            .attr({
                ...
                "type":"text",
                "value":this.getNowTime(false,addMinute)
            })
            .appendTo(this.div)
            .datetimepicker(
            {
    ...
    /**
    * @author:noname,wangkun
    * @date:2017-05-02
    * @param:isZh-中国;addMinute-增加分钟
    * @return:时间
    * @description:获取时间(最小到分钟)
    */
    this.getNowTime = function(isZh,addMinute){
        var date = new Date();
    ...
    */
    this.getCurrentDateTime = function(){
        ...
        var day = parseInt(curTimeStr.replace(/\d*-\d*-(\d*) \d*:\d*:\d*/,"$1"));
        var hour = parseInt(curTimeStr.replace(/\d*-\d*-\d* (\d*):\d*:\d*/,"$1"));
        var minutes = parseInt(curTimeStr.replace(/\d*-\d*-\d* \d*:(\d*):\d*/,"$1"));
        var seconds = 0;
        var date = new Date();
        date.setFullYear(year,month - 1,day);
        date.setHours(hour, minutes, seconds, 0);
