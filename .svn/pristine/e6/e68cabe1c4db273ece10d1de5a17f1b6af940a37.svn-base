 ...
 */
function GridDidplayPageClass(){
    this.myDateSelecter = null;
    this.yubaoshixiaoTools = null;
    this.currentPosition = {lon:108.349,lat:22.82}; //南宁本站（南宁城区站）
    this.stations = [{lon:108.349,lat:22.82},{lon:107.93,lat:21.84},{lon:110.47,lat:25.29},{lon:109.02,lat:22.12},{lon:109.38,lat:24.43},{lon:106.56,lat:24.07}
        ,{lon:109.51,lat:23.7},{lon:111.03,lat:23.27},{lon:111.3,lat:24.34},{lon:109.8,lat:23.14},{lon:107.81,lat:24.6},{lon:107.11,lat:22.32},{lon:109.33,lat:21.65},{lon:110.34,lat:22.52}];
    this.num = 0;
    this.polling = true;
    var t = this;
    ...
    this.renderMenu = function(){
        ...
        $("#map_div").css("height", (parseInt(height)-parseInt(heightMapTool))+"px");
        ...
        $("#latticeControl").click(function(){
            if($(this).attr("title") == "隐藏"){
                ...
                $("#mapContainer_div").css("width","100%");
                //GDYB.Page.curPage.map.updateSize();
                GDYB.Page.curPage.map.updateSize();
                $(this).removeClass("latticeControlShow");
                $(this).addClass("latticeControlHide");
            }
            else{
                $("#latticeForcast").show();
                $(this).attr("title","隐藏").html("＞");
                $("#mapContainer_div").css("width","59%");
                //GDYB.Page.curPage.map.updateSize();
                GDYB.Page.curPage.map.updateSize();
                $(this).removeClass("latticeControlHide");
                $(this).addClass("latticeControlShow");
            }
        });

        //表格的显示隐藏
        $(".bottomPanelImg").click(function(){
            if($(this).attr("flag") == "noActive"){
                $(this).attr("flag","active").css("bottom","230px");
                $(this).find("img").attr("src","imgs/top.png");
                $("#top_div").css("height",parseInt(height)-230);
                $("#map_div").css("height", parseInt(height)-parseInt(heightMapTool)-230);
                //GDYB.Page.curPage.map.updateSize();
                GDYB.Page.curPage.map.updateSize();
                $("#bottom_div").css("display","block");
            }
            else{
                $(this).attr("flag","noActive").css("bottom","2px");
                $(this).find("img").attr("src","imgs/bottom.png");
                $("#top_div").css("height",height);
                $("#map_div").css("height", parseInt(height)-parseInt(heightMapTool));
                //GDYB.Page.curPage.map.updateSize();
                GDYB.Page.curPage.map.updateSize();
        ...
        map.events.register("click", map, function(event){
            ...
            showGridProduct(this.getLonLatFromPixel(event.xy));
        });
        ...
        $("#latticeForcast").find("canvas").attr("width",(parseInt($("#latticeForcast").css("width"))-20));
    ...
    //轮询
    function pollingShowGridProduct(){
        setTimeout(function(){
            if(t.polling){
                if(t.num<13)
                    t.num++;
                    ...
                    t.num = 0;
                showGridProduct(t.stations[t.num]);
            }
            pollingShowGridProduct();
        },5000)
    }

    function showGridProduct(obj){
        t.currentPosition = obj;
...
GridDidplayPageClass.prototype = new PageBase();