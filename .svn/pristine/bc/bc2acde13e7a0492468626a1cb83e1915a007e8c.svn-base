...
~ function($) {
	 ...
	 * @author rexer
	 * @date   2017-04-20
	 * @param  {Array}   paras  参数 areaCode:区域代码; level:行政级别(省,市,县) `prvn`,`cty`,`cnty`
	 * @return {Promise}
	 */
	T.downAreas = function(paras) {
		var defer = $.Deferred();
		var Query = new T.DeferredQuery();

		var url = gridServiceUrl + 'services/AdminDivisionService/getDivisionInfos';
		// localhost
		// var url = T.url('WMGridService', 'AdminDivisionService', 'getDivisionInfos');

		var reqSize = paras.length;
		for (var i = 0; i < reqSize; i++) {
			Query.addQuery(url, T.paramize(paras[i]));
		}
