...
function ZDYBPageClass(){
    ...
    this.renderMenu = function(){
        var htmlStr = "<div id='div_datetime' style='padding-left:10px;height: 40px;background-color: rgba(1,183,255,0.63);'>"
            +"<div id='dateSelect' style='margin: 10px 0px 0px 0px;float: left;width: 140px;height: 26px;'></div>" +
            "<div id='zdybTimeTypeDiv' class='zdybTimeTypeDiv'><input type='radio' name='zdybtime' checked='true' style=''><span class='zdybTimeType'>全部</span>" +
            "<input type='radio' name='zdybtime' style=''><span class='zdybTimeType'>早晨</span>" +
            "<input type='radio' name='zdybtime' style=''><span class='zdybTimeType'>上午</span>" +
            "<input type='radio' name='zdybtime' style=''><span class='zdybTimeType'>下午</span></div></div>"
        ...
        if($("#workspace_div").find("#ZDYBDiv").length == 0){
            $("#workspace_div").append("<div id='ZDYBDiv' style='background-color: rgb(240,240,240);left:415px;right:0px;top: 4px;bottom: 0px;position: absolute;'><div id='ZDYBAllContent'>" +
                ...
                "<div class='zdybRelationClass' flag='false' style='display: none;' name='station'>站点关联</div>" +
                "<div class='zdybRelationClass' flag='false' style='display: none;' name='time'>时效关联</div>" +
                "</div>" +
                ...
                "</div>"+
                "</div>" +
                "<div id='zdybPreviewDiv' style='width: 100%;display: none;overflow: auto;background-color: #FFFFFF;position: absolute;top: 61px;bottom: 0px;'>" +
                "</div>" +
                "</div>" +
                "</div>");
        }
        ...
        $("#zdybShowChange").click(function(){
            if($(this).attr("name")=="zdybPreviewButton"){
                var textShow = getCheckText();
                if(textShow != ""){
                    var result = confirm(textShow);
                    if(result == false){
                        return;
                    }
                }
    ...
    function initZDYBTable(list){
        ...
        for(var i=0;i< t.elements.length;i++){
            if(t.elements[i].element == "w"){
                colorClass = "zdybWeather1";
            }
            else if(t.elements[i].element.indexOf("wd")!=-1){
                colorClass = "zdybWeather2";
            }
            else if(t.elements[i].element.indexOf("ws")!=-1){
                colorClass = "zdybWeather3";
            }
        ...
        if((hIn>hOUt&&wIn>(wOUt-17))||(wIn>wOUt&&hIn>(hZDYBDiv-77))){
            ...
            $("#scrollX").css("display","block");
            $("#scrollY").css("display","block");
            ...
            $("#zdybHeaderdiv").css("right","17px");
            $("#zdybMaindiv").css("right","17px");
            $("#zdybColumndiv").css("bottom","0px");
            $("#zdybMaindiv").css("bottom","0px");
            $("#scrollX").css("display","none");
            $("#scrollY").css("display","block");
            $("#scrollY").css("right",wOUt-wIn);
            $("#scrollY").css("bottom","0px");
        }
            ...
            for(var i=0;i<data.items.length;i++){
                ...
                if( elementStr=="1") {
                    for(var j=0;j<data.items[i].datas.length;j++) {
                        //可输入
                        contentObj[data.items[i].element +"_"+ data.items[i].hourSpan] += '<td class="zdybElement zdybElement' + j + '" cols="' + j + '" value="' + getelementValue(data.items[i].datas[j]) + '" type="' + data.items[i].element + '"><input type="text" class="zdybTemp" value="' + getelementName(data.items[i].element, data.items[i].datas[j]) + '"></td>';
                    }
                }
                else{
                    for(var j=0;j<data.items[i].datas.length;j++) {
                ...
                if((contentObj[obj].indexOf("cols")==-1)&&obj!="0"&&obj!="1"){
                    if(t.allElements[obj.split("_")[0]].input=="1"){
                        for(var j=0;j<data.stationNums.length;j++) {
                            //温度可输入
                            contentObj[obj] += '<td class="zdybElement zdybElement' + j + '" cols="' + j + '" type="' + obj.split("_")[0] + '" value="999.9"><input type="text" class="zdybTemp" value=""></td>';
                        }
                    }
                    else{
                        for(var j=0;j<data.stationNums.length;j++) {
        ...
        $(".zdybElement").click(function(event){
            ...
            switch(t.allElements[$(this).attr("type")].element){
                ...
                case "wmax":
                    ...
                    break;
                case  "10uv":
                    if($(this).attr("type").indexOf("wd") != -1){
                        contentHtml += '<li value="1">东北风</li>'+
                            '<li value="2">东风</li>'+
                            '<li value="3">东南风</li>'+
                            '<li value="4">南风</li>'+
    ...
    function getZDYBPublishTime(){
		// GDYB.GridProductClass.currentUserDepart.departCode = 45;
        $.ajax({
            type: 'post',
    ...
    function grid2station(){
		...
		// t.nowElement.gdybType = 'prvn';
        var url=getGridServiceUrl(gridMakeTime)+"services/GridService/grid2station";
        var para = "{departCode:'" + GDYB.GridProductClass.currentUserDepart.departCode
			+"',productId:'" + t.nowElement.id
			+"',type:'" + t.nowElement.gdybType
			+"',stationType:'" + t.nowElement.stationType
			+"',makeTime:'" + gridMakeTime
        ...
        $.ajax({
            ...
            success:function(data) {
                t.nowData = data;
                t.xzData = {};
                if(t.nowElement.productName == "大城市精细化预报"){
                    for(var i=0;i< t.nowData.items.length;i++){

                        if(t.nowData.items[i].element == "w6" && (t.nowData.items[i].hourSpan%12 == 0)){
                            t.nowData.items.push({element:"w6",hourSpan:(t.nowData.items[i].hourSpan-6),datas:t.nowData.items[i].datas});
                        }
                    }
                }
    ...
    function getServiceTime(num){
        $.ajax({
            ...
            success: function (data) {
                ...
                if(num==0){
                    var nowTime = $("#dateSelect").find("input").val();
                    var nowDate = new Date(data.replace("/-/g","/"));
                    var endDate;
                    ...
                    for(var i=0;i<list.length;i++){
                        endtime = t.elementData[parseInt($(list[i]).attr("elementId"))].endTime;
                        if(time< endtime){
                            endDate = new Date(nowTime.replace("/-/g","/")+" "+endtime);
                            $(list[i]).click();
                            allPast = false;
                            break;
                        }
                    ...
                    if(allPast&&list.length!=0){
                        endtime = t.elementData[parseInt($(list[list.length-1]).attr("elementId"))].endTime;
                        endDate = new Date(nowTime.replace("/-/g","/")+" "+endtime);
                    ...
                    var endDate = new Date(nowTime.replace("/-/g","/")+" "+endtime);
                    t.endTime = (endDate.getTime()-nowDate.getTime())/1000;
                }
            }
        });
    ...
    function getelementValue(value,j){
        ...
        if(typeof (j)!="undefined"){
            if(typeof(value)!="undefined"&& value[j]!=-9999&&value[j]!="-9999"){
                if(t.nowElement.productName.indexOf('城镇预报') > -1 || t.nowElement.productName.indexOf('大城市') > -1){
    ...
    function getelementName(element,value){
        ...
        switch (element){
            case "w":
			case "w6": //add by pope on 20170510
                name = getWeatherName(parseInt(value));
                break;
            case "wd":
			case "wd6": //add by pope on 20170510
                name = getWDName(parseInt(value));
                break;
            case "ws":
			case "ws6": //add by pope on 20170510
    ...
    function getZDYBForecastNum(){
        ...
        var nowDate = new Date(nowTime.replace("/-/g","/")+" "+t.nowElement.makeTime);
        ...
        var productNameDetail = "精细化报文";
        if(t.areaName == "qutai"){
            if(t.nowElement.postType != "r")
                productNameDetail = "省台精细化报文/首席岗";
            else
                productNameDetail = "省台精细化报文/领班岗"
        }
        var productZD = "指导预报";
        if(t.areaName == "qutai")
            productZD = "省台"+productZD;
        var productName1 = productAreaName+"/"+t.nowElement.productName+"/"+productNameDetail;
        var productName2 = productAreaName+"/"+t.nowElement.productName+"/"+productZD;
        ...
        $.ajax({
            ...
            success: function (list) {
                ...
                for(var i=0;i<data.length;i++){
                    ...
                    var list = [];
                    var nowTime = $("#dateSelect").find("input").val();
                    var nowDate = new Date(nowTime.replace("/-/g","/")+" "+data[i].makeTime);
                    nowDate.setHours(nowDate.getHours()-8);
                    for(var j=0;j<outlist.length;j++){
                        if(outlist[j]=="1"){//城镇预报
                            ...
                            if(t.areaName == "qutai"){
                                if(data[i].postType != "r")
                                    obj.productName = productAreaName+"/"+data[i].productName+"/省台"+outTypeObj[outlist[j]]+"/首席岗";
                                else
                                    obj.productName = productAreaName+"/"+data[i].productName+"/省台"+outTypeObj[outlist[j]]+"/领班岗";
                            }
                            obj.name = "Z_SEVP_C_" + GDYB.GridProductClass.currentUserDepart.codeOfTownForecast + "_" + makeTime + ".*?_P_RFFC-SPCC-" + time + ".TXT";
                            list.push(obj);
                        }
                        else if(outlist[j]=="2"){//指导预报
                            ...
                            var time = nowTime.substr(0,4)+nowTime.substr(5,2)+nowTime.substr(8,2)+data[i].forecastTime+"00-"+(Array(2).join(0)+data[i].hourSpanTotal).slice(-3)+ "12";
                            var obj = {};
                            obj.productName = productAreaName+"/"+data[i].productName+"/"+outTypeObj[outlist[j]];
                            obj.name = "Z_SEVP_C_"+GDYB.GridProductClass.currentUserDepart.codeOfTownForecast+"_"+makeTime+".*?_P_RFFC-GXXZ-"+time+".TXT";
                            list.push(obj);
                        }
                        else if(outlist[j]=="4"){//空气污染
                            var makeTime = nowDate.getFullYear()+(Array(2).join(0)+(nowDate.getMonth()+1)).slice(-2)+(Array(2).join(0)+nowDate.getDate()).slice(-2)+(Array(2).join(0)+nowDate.getHours()).slice(-2)+(Array(2).join(0)+nowDate.getMinutes()).slice(-2);
                            var time = nowTime.substr(0,4)+nowTime.substr(5,2)+nowTime.substr(8,2)+data[i].forecastTime+"00-"+(Array(2).join(0)+data[i].hourSpanTotal).slice(-3)+ data[i].hourSpan;
                            var obj = {};
                            obj.productName = productAreaName+"/"+data[i].productName+"/"+outTypeObj[outlist[j]];
                        ...
                        else if(outlist[j]=="8"){//大城市预报
                            var makeTime = nowDate.getFullYear()+(Array(2).join(0)+(nowDate.getMonth()+1)).slice(-2)+(Array(2).join(0)+nowDate.getDate()).slice(-2)+(Array(2).join(0)+nowDate.getHours()).slice(-2)+(Array(2).join(0)+nowDate.getMinutes()).slice(-2);
                            var time = nowTime.substr(0,4)+nowTime.substr(5,2)+nowTime.substr(8,2)+data[i].forecastTime+"00-"+(Array(2).join(0)+data[i].hourSpanTotal).slice(-3)+ (Array(2).join(0)+data[i].hourSpan).slice(-2);
                            var obj = {};
                            obj.productName = productAreaName+"/"+data[i].productName+"/"+outTypeObj[outlist[j]];
                            obj.name = "Z_SEVP_C_BEJN_" + makeTime + "00_P_RFFC-SPCC6H-" + time + ".TXT";
                            list.push(obj);
                        }
                        else if(outlist[j]=="9"){//雨量预报
                            var obj = {};
                            obj.productName = productAreaName+"/"+data[i].productName+"/"+outTypeObj[outlist[j]];
                            obj.name = "M"+nowTime.substr(5,2)+nowTime.substr(8,2)+data[i].forecastTime+".F"+GDYB.GridProductClass.currentUserDepart.codeOfGuidanceForecast;
                            list.push(obj);
                        }
                    }
                    nameList.push(list);
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        $("#zdybProductTypePanel").find("div.dis_menu_body_item").find("span").remove();
                        var list = $("#zdybProductTypePanel").find("div.dis_menu_body_item");
                        for(var i=0;i<data.length;i++){
    ...
    function zdybPreviewTown(typeNum){
    	debugger
        var nowTime = $("#dateSelect").find("input").val(),num = '01';
        ...
        var nameMakeTime = nowDate.getFullYear()+(Array(2).join(0)+(nowDate.getMonth()+1)).slice(-2)+(Array(2).join(0)+nowDate.getDate()).slice(-2)+(Array(2).join(0)+nowDate.getHours()).slice(-2)+(Array(2).join(0)+nowDate.getMinutes()).slice(-2);
        var forecastTime = nowTime.substr(0,4)+nowTime.substr(5,2)+nowTime.substr(8,2)+t.nowElement.forecastTime+"00-"+(Array(2).join(0)+t.nowElement.hourSpanTotal).slice(-3)+ t.nowElement.hourSpan;
        // var name = "Z_SEVP_C_BEJN_" + GDYB.GridProductClass.currentUserDepart.codeOfTownForecast + "_" + nameMakeTime + (Array(2).join(0)+ t.zdybTownNum).slice(-2) + "_P_RFFC-SPCC-" + forecastTime + ".TXT";
		var name = time + num + ".TXT";
        ...
        var contentHtml = "<div style='text-align: center;'>"+name+"</div>";
        contentHtml += '<div>ZCZC</div>';
        contentHtml += '<div>FSCI50 BEJN'+GDYB.GridProductClass.currentUserDepart.codeOfTownForecast+' '+makeTime+" "+modify+'</div>';
        contentHtml += '<div>'+time+'时'+GDYB.GridProductClass.currentUserDepart.departName+'气象台预报产品'+showName+'</div>';
        ...
        for(var j=0;j<stationLength;j++){
            ...
            contentHtml += '<table class="zdybPreviewTable"><tr><td>'+t.nowData.stationNums[j]+' '+Longitude+' '+Latitude+' '+(Array(6).join(" ")+hHeight).slice(-6)+' '+yubaoshixiao+' 21</td></tr></table><table class="zdybPreviewTable">';
    ...
    function zdybPreviewGuide(typeNum){
        ...
        var forecastTime = nowTime.substr(8,2)+t.nowElement.zdybHour;
        var name = GDYB.GridProductClass.currentUserDepart.codeOfGuidanceForecast + "DY" + forecastTime+ ".ENN";
        var contentHtml = "<div style='text-align: center;'>"+name+"</div>"
        +"<div id='zdybGuideTitle' style='width: 100%;padding-left: 300px;'>"+GDYB.GridProductClass.currentUserDepart.departName+"气象台"+time+"天气预报</div>"
        +"<div style='padding: 10px;'>一、天气形势和趋势预报</div>"
        +"<div id='zdybGuideTrendDetail'><textarea id='zdybGruideTextArea' style='width: 751px;margin-left: 10px;background-color: #fff;border-color: #FFFFFF;box-shadow:none;' disabled='disabled'></textarea></div>"
        +"<div style='padding: 10px;'>二、分县要素预报</div>"
        +"<div id='zdybGuideElementDetail'>";
        ...
        for(var i= 24;i<= hourSpanTotal;){
            ...
            for(var j=0;j<staionLength;j++){
                ...
                if(getelementValue(t.xzData["wd_"+(i-12)],j)==getelementValue(t.xzData["wd_"+i],j) && getelementValue(t.xzData["ws_"+(i-12)],j)==getelementValue(t.xzData["ws_"+i],j)){
                    var wValue = getelementName1("wd", t.xzData["wd_"+i],j)+getelementName1("ws", t.xzData["ws_"+i],j);
                }
                else if(getelementValue(t.xzData["wd_"+(i-12)],j)==getelementValue(t.xzData["wd_"+i],j)){
                    var wValue = getelementName1("wd", t.xzData["wd_"+i],j)+getelementName1("ws", t.xzData["ws_"+(i-12)],j)+'转'+getelementName1("ws", t.xzData["ws_"+i],j);
                }
                else if(getelementValue(t.xzData["ws_"+(i-12)],j)==getelementValue(t.xzData["ws_"+i],j)){
                    var wValue = getelementName1("wd", t.xzData["wd_"+(i-12)],j)+'转'+getelementName1("wd", t.xzData["wd_"+i],j)+getelementName1("ws", t.xzData["ws_"+i],j);
                }
                else{
                    var wValue = getelementName1("wd", t.xzData["wd_"+(i-12)],j)+getelementName1("ws", t.xzData["ws_"+(i-12)],j)+'转'+getelementName1("wd", t.xzData["wd_"+i],j)+getelementName1("ws", t.xzData["ws_"+i],j);
                }
                contentHtml += '<td><input class="zdybTemp" disabled="disabled" value="'+wValue+'"></td>';
    ...
    function zdybPreviewCountry(typeNum){
        ...
        $.ajax({
            ...
            success: function (data) {
                var productNum = data.num;
                var showName = "Z_SEVP_C_"+GDYB.GridProductClass.currentUserDepart.codeOfTownForecast+"_"+makeTime+(Array(2).join(0)+productNum).slice(-2)+"_P_RFFC-GXXZ-"+yubaoTime+".TXT";
                var flag = "";
                if(data.num == 0){
                    if(t.endTime<0){
                        flag = "RRA";
                    ...
                    else if (data.name.indexOf("CC") != -1) {
                        if (t.endTime < 0) {
                            flag = "RRA"
                        }
                        else {
                            var fileNum = data.name.split("CC")[1];
                            flag = "CC" + String.fromCharCode(fileNum.charCodeAt() + 1);
                        }
                    }
                    else if (data.name.indexOf("RR") != -1) {
                        var fileNum = data.name.split("RR")[1];
                ...
                if(typeof (t.nowData) != "undefined"){
                    ...
                    var contentHtml = "<div style='text-align: center;'>"+showName+"</div>";
                    ...
                    for(var j=0;j< stationnum;j++){
                        ...
                        for(var k=12;k<169;){
                            contentHtml += '<tr><td>'+k+'</td><td>'+getelementValue(t.xzData["temp3_"+k],j)+'</td><td>'+getelementValue(t.xzData["rh3_"+k],j)+'</td><td>'+getelementValue(t.xzData["wdNum3_"+k],j)+'</td><td>'+getelementValue(t.xzData["wsNum3_"+k],j)+'</td><td>999.9</td><td id="r12_'+k+'_'+j+'">'+getelementValue(t.xzData["r12_"+k],j)+'</td><td>999.9 999.9 999.9 999.9</td>';
                            if(k%24==0){
                                contentHtml +='<td id="tmax_'+k+'_'+j+'">'+getelementValue(t.xzData["tmax_"+k],j)+'</td><td id="tmin_'+k+'_'+j+'">'+getelementValue(t.xzData["tmin_"+k],j)+'</td><td>'+getelementValue(t.xzData["rhmax_"+k],j)+'</td><td>'+getelementValue(t.xzData["rhmin_"+k],j)+'</td><td id="r24_'+k+'_'+j+'">'+getelementValue(t.xzData["r24_"+k],j)+'</td>';
                            }
                            else{
                                contentHtml +='<td>999.9</td><td>999.9</td><td>999.9</td><td>999.9</td><td>999.9</td>';
                            }
                            if(k%12==0){
                                contentHtml +='<td id="r12_'+k+'_'+j+'">'+getelementValue(t.xzData["r12_"+k],j)+'</td><td>999.9</td><td>999.9</td><td>'+getelementValue(t.xzData["w_"+k],j)+'</td><td>'+getelementValue(t.xzData["wd_"+k],j)+'</td><td>'+getelementValue(t.xzData["ws_"+k],j)+'</td>';
                            }
                            else{
                    ...
                    $("#zdybPreviewDiv"+typeNum).html(contentHtml);
                    var checkData = getCheckText("xzyb");
                    setTimeout(function(){
                        if(checkData.textShow != "")
                            alert(checkData.textShow);
                        if(checkData.list.length != 0){
                            $("#"+checkData.list[0].element).css("background-color","red");
                            var a = document.createElement('a');
                            a.style.display = 'none';
                            a.setAttribute('href', "#"+checkData.list[0].element);
                            document.body.appendChild(a);
                            a.dispatchEvent(new MouseEvent('click'));
                            document.body.removeChild(a);
                            $("body").append('<div class="zdybCheck"><label title="关闭">×</label><span flag = "0">'+checkData.list[0].value+'</span><div style="margin-right: 85px;">上一个</div><div>下一个</div></div>');
                            $(".zdybCheck").find("label").click(function(){
                                var flag = parseInt($(this).parent().find("span").attr("flag"));
                                $("#"+checkData.list[flag].element).css("background-color","");
                                $(".zdybCheck").remove();
                            });
                            $(".zdybCheck").find("div").click(function(){
                                var flag = parseInt($(this).parent().find("span").attr("flag"));
                                $("#"+checkData.list[flag].element).css("background-color","");
                                var startNum = parseInt(parseInt(checkData.list[flag].element.split("_")[2])/100);
                                if($(this).html() == "上一个"){
                                    if(flag > 0){
                                        flag--;
                                        $(this).parent().find("span").attr("flag",flag);
                                        $(this).parent().find("span").html(checkData.list[flag].value);
                                    }
                                }
                                else{
                                    if(flag < checkData.list.length-1){
                                        flag++;
                                        $(this).parent().find("span").attr("flag",flag);
                                        $(this).parent().find("span").html(checkData.list[flag].value);
                                    }
                                }
                                var endNum = parseInt(parseInt(checkData.list[flag].element.split("_")[2])/100);
                                if(startNum != endNum)
                                    $(".xzybPageDiv").eq(endNum).click();
                                $("#"+checkData.list[flag].element).css("background-color","red");
                                var a = document.createElement('a');
                                a.style.display = 'none';
                                a.setAttribute('href', "#"+checkData.list[flag].element);
                                document.body.appendChild(a);
                                a.dispatchEvent(new MouseEvent('click'));
                                document.body.removeChild(a);
                            });
    ...
    function zdybPreviewSea(typeNum){
        ...
        var name = "HY"+yubaoTime+".F"+GDYB.GridProductClass.currentUserDepart.codeOfGuidanceForecast;
        var contentHtml = "<div style='text-align: center;'>"+name+"</div><table class='zdybPreviewTable'><tr><td>"+t.stationList[0].AreaName+"气象台海洋预报</td></tr></table><table class='zdybPreviewTable'>";
        for(var j= parseInt(t.nowElement.hourSpan);j<= parseInt(t.nowElement.hourSpanTotal);){
            contentHtml +="<tr><td>"+j+"</td>" +
    ...
    function zdybPreviewCoast(typeNum){
        var nowTime = $("#dateSelect").find("input").val();
        var yubaoTime = nowTime.substr(0,4)+nowTime.substr(5,2)+nowTime.substr(8,2)+t.nowElement.forecastTime;
        var showName = "SEVP_GXMA_ROFC_SFER_EME_AGX_L89_P9_"+yubaoTime+"00";
        var contentHtml = "";
        for(var i=24;i<=72;){
            var time = nowTime.substr(2,2)+"年"+nowTime.substr(5,2)+"月"+nowTime.substr(8,2)+"日"+ (parseInt(t.nowElement.forecastTime)+8)+"时"+i+"小时山东气象台沿岸海区预报";
            ...
            if(i==24){
                contentHtml += "<span style='display: block;text-align: center;'>"+showName+"0"+i+"00.MIC</span>";

            }
            else{
                contentHtml += "<span style='display: block;text-align: center;margin-top: 50px;'>"+showName+"0"+i+"00.MIC</span>";
            }
    ...
    function zdybPreviewBigCity(typeNum){
        ...
        var showMakeTime = nowDate.getFullYear()+(Array(2).join(0)+(nowDate.getMonth()+1)).slice(-2)+(Array(2).join(0)+nowDate.getDate()).slice(-2)+(Array(2).join(0)+nowDate.getHours()).slice(-2)+(Array(2).join(0)+nowDate.getMinutes()).slice(-2);
        var showTime = nowTime.substr(0,4)+nowTime.substr(5,2)+nowTime.substr(8,2)+t.nowElement.forecastTime+"00-"+(Array(2).join(0)+t.nowElement.hourSpanTotal).slice(-3)+ (Array(2).join(0)+t.nowElement.hourSpan).slice(-2);
        var name = "Z_SEVP_C_BEJN_" + showMakeTime + "00_P_RFFC-SPCC6H-" + showTime + ".TXT";
        var contentHtml = "<div style='text-align: center;'>"+name+"</div>";
        contentHtml += '<div>ZCZC</div>';
        contentHtml += '<div>FSCI50 BEJN '+makeTime+'</div>';
        contentHtml += '<div>'+time+'时山东'+GDYB.GridProductClass.currentUserDepart.departName+'6小时精细化预报</div>';
        ...
        for(var j=0;j<t.stationList.length;j++){
            ...
            for(var i= parseInt(t.nowElement.hourSpan);i<= parseInt(t.nowElement.hourSpanTotal);){
                contentHtml += '<tr><td>'+(Array(3).join(0)+i).slice(-3)+'</td><td>'+ parseInt(getelementValue(t.xzData["w6_"+i],j))+'</td><td>'+ getelementValue(t.xzData["tmax6_"+i],j)+'</td><td>'+ getelementValue(t.xzData["tmin6_"+i],j)+'</td>' +
                    '<td>'+ parseInt(getelementValue(t.xzData["wd6_"+i],j)) +'</td><td>'+ parseInt(getelementValue(t.xzData["ws6_"+i],j))+'</td><td>'+ (getelementValue(t.xzData["r6_"+i],j)<0.1?999.9:getelementValue(t.xzData["r6_"+i],j))+'</td></tr>';
                i += parseInt(t.nowElement.hourSpan);
            }
            contentHtml += '</table>';
        }
    ...
    function zdybUploadTown(obj){
        var typeNum = $(obj).attr("type"),num='01';
        var nowTime = $("#dateSelect").find("input").val();
        var makeTimeList = t.nowElement.makeTime.split(":");
        var nowDate = new Date(nowTime.replace("/-/g","/")+" "+t.nowElement.makeTime);
		if(nowDate.getHours() > 12){
			num = '02';
		}
            ...
            for(var j=0;j<listTr.length;j++){
                ...
                for(var k=0;k<listTd.length;k++){
                    if(k==0){
                        ...
                        content += formatForecast($(listTd[k]).html(),4)+ " ";
                    }
                    else{
        ...
        // var name = makeTime + (Array(2).join(0)+ t.zdybTownNum).slice(-2) + ".TXT";
		var name = nowTime.substr(0,4)+nowTime.substr(5,2)+nowTime.substr(8,2)+ t.nowElement.forecastTime + num + ".TXT";
        var productName = "";
        if(t.areaName == "qutai")
            productName += "省台";
        productName += $(obj).attr("name")+"";
        if(t.areaName == "qutai"){
            if(t.nowElement.postType != "r")
                productName += "/首席岗";
            else
		...
		productName = czybPath;
        zdybUploadTemplate(productName,name,content,getZDYBForecastNum)
    }
    ...
    function zdybUploadGuide(obj){
        ...
        var listDiv = $("#zdybPreviewDiv"+typeNum).find("div");
        var listTable = $("#zdybPreviewDiv"+typeNum).find("table");
        content += $(listDiv[1]).html()+"\r\n"+$(listDiv[2]).html()+"\r\n"+$(listDiv[3]).find("textarea").val()+"\r\n"+$(listDiv[4]).html()+"\r\n";
        for(var i=0;i<listTable.length;i++){
            content += $(listDiv[i+6]).html()+"\r\n";
            var listTr = $(listTable[i]).find("tr");
            for(var j=0;j<listTr.length;j++){
                var listTd = $(listTr[j]).find("td");
                for(var k=0;k<listTd.length;k++){
                    if($(listTd[k]).find("input").length==0){
                        content += $(listTd[k]).html()+Array(30-strlen($(listTd[k]).html())).join(" ");
                    }
                    else{
                        content += $(listTd[k]).find("input").val()+Array(30-strlen($(listTd[k]).find("input").val())).join(" ");
                    }
                }
                content += "\r\n";
            }
    ...
    function zdybUploadCountry(obj){
        ...
        var makeTime = nowDate.getFullYear()+(Array(2).join(0)+(nowDate.getMonth()+1)).slice(-2)+(Array(2).join(0)+nowDate.getDate()).slice(-2)+(Array(2).join(0)+nowDate.getHours()).slice(-2)+(Array(2).join(0)+nowDate.getMinutes()).slice(-2);
        var time = nowTime.substr(0,4)+nowTime.substr(5,2)+nowTime.substr(8,2)+t.nowElement.forecastTime;
        var searchName = name = "Z_SEVP_C_"+GDYB.GridProductClass.currentUserDepart.codeOfTownForecast+"_"+makeTime+".*?_P_RFFC-GXXZ-"+time+"00-16812.TXT";
        var yubaoshixiaoObject = {};
        for(var i=0;i< t.elements.length;i++){
            yubaoshixiaoObject[t.elements[i].hourSpan] = 1;
        }
        ...
        var productAreaName = "";
        var productName = productAreaName+"/"+t.nowElement.productName+"/"+"乡镇报文";
        var param = '{"name":"'+searchName+'","productName":"'+productName+'"}';
        ...
        $.ajax({
            ...
            success: function (data) {
                var flag = "";
                var productNum = data.num;
                var name = "Z_SEVP_C_"+GDYB.GridProductClass.currentUserDepart.codeOfTownForecast+"_"+makeTime+(Array(2).join(0)+productNum).slice(-2)+"_P_RFFC-GXXZ-"+time+"00-16812.TXT";
                if(data.num == 0){
                    if(t.endTime<0){
                        flag = "RRA";
                    }
                ...
                for(var j=0;j< t.stationList.length;j++){
                    ...
                    for(var k=12;k<169;){
                        content += formatForecast(k.toString(),3)+' '+formatForecast(getelementValue(t.xzData["temp3_"+k],j))+' '+formatForecast(getelementValue(t.xzData["rh3_"+k],j))+' '+formatForecast(getelementValue(t.xzData["wdNum3_"+k],j))+' '+formatForecast(getelementValue(t.xzData["wsNum3_"+k],j))+' 999.9 '+formatForecast(getelementValue(t.xzData["r12_"+k],j))+' 999.9 999.9 999.9 999.9 ';
                        if(k%24==0){
    ...
    function zdybUploadBigCity(obj){
        ...
        content = content.substr(0,content.length-3);
        content += "\r\n\r\n";
        var name = "Z_SEVP_C_BEJN_" + makeTime + "00_P_RFFC-SPCC6H-" + time + ".TXT";
		var productName = dcsybPath || $(obj).attr("name");
        zdybUploadTemplate(productName,name,content)
    }
    //雨量预报上网
    ...
    function zdybUploadTemplate(productName,name,content,func){
    	debugger
		if(GDYB.GridProductClass.currentUserName == "fuwuzhongxin" || GDYB.GridProductClass.currentUserName == "guest"){ //服务中心和宾客无提交权限，临时写在前端
			alert("无提交权限");
			return;
		}
    ...
    function getCheckText(elementName){
        var textShow = "";
        if(t.nowElement.productName == "城镇预报"){
            if($("#zdybShowChange").attr("name")=="zdybPreviewButton"){
                var weatherObj = {};
                var list = $(".ZDYBTable").find("td[type='w']");
                var name = "";
                for(var i=0;i<list.length;i++){
                    name = "";
                    if($(list[i]).html().indexOf("雪")!=-1)
                        name = "雪";
                    else if($(list[i]).html().indexOf("冰雹")!=-1)
                        name = "冰雹";
                    else if($(list[i]).html().indexOf("冻雨")!=-1)
                        name = "冻雨";
                    else if($(list[i]).html().indexOf("雾")!=-1)
                        name = "雾";
                    else if($(list[i]).html().indexOf("霾")!=-1)
                        name = "霾";
                    else if($(list[i]).html().indexOf("浮尘")!=-1)
                        name = "浮尘";
                    else if($(list[i]).html().indexOf("扬沙")!=-1)
                        name = "扬沙";
                    else if($(list[i]).html().indexOf("沙尘")!=-1)
                        name = "沙尘";
                    if(name == "")
                        continue;
                    if(!weatherObj.hasOwnProperty(name)){
                        weatherObj[name] = [list[i]];
                    }
                    else{
                        weatherObj[name].push(list[i]);
                    }
                }
                for(var name in weatherObj){
                    for(var i=0;i<weatherObj[name].length;i++){
                        if(i==2){
                            textShow += "...\n";
                            break;
                        }
                        textShow += t.stationList[parseInt($(weatherObj[name][i]).attr("cols"))].StationName + "站";
                        textShow += $(weatherObj[name][i]).parent().attr("shixiao") + "小时预报:" +$(weatherObj[name][i]).html()+"\n";
                    }
                    if(weatherObj[name].length>0){
                        textShow += "共" + weatherObj[name].length + "站次预报包含"+name+"\n";
                    }
                }
                var listTmin =  $(".ZDYBTable").find("td[type='tmin']");
                var listTmax =  $(".ZDYBTable").find("td[type='tmax']");
                var listTemp = [];
                var listTempMin = [];
                var listTempMax = [];
                for(var i=0;i<listTmin.length;i++){
                    if($(listTmin[i]).find("input").val()!=""&&$(listTmax[i]).find("input").val()!=""&&parseFloat($(listTmin[i]).find("input").val())>parseFloat($(listTmax[i]).find("input").val()))
                        listTemp.push(listTmin[i]);
                    if($(listTmin[i]).find("input").val()!=""&&parseFloat($(listTmin[i]).find("input").val())<=0)
                        listTempMin.push(listTmin[i]);
                    if($(listTmax[i]).find("input").val()!=""&&parseFloat($(listTmax[i]).find("input").val())<=0)
                        listTempMin.push(listTmax[i]);
                    if($(listTmin[i]).find("input").val()!=""&&parseFloat($(listTmin[i]).find("input").val())>=40)
                        listTempMax.push(listTmin[i]);
                    if($(listTmax[i]).find("input").val()!=""&&parseFloat($(listTmax[i]).find("input").val())>=40)
                        listTempMax.push(listTmax[i]);
                }
                ...
                for(var i=0;i<listTemp.length;i++){
                    ...
                    textShow += t.stationList[parseInt($(listTemp[i]).attr("cols"))].StationName + "站";
                    textShow += $(listTemp[i]).parent().attr("shixiao") + "小时预报:" +"低温>高温\n";
                }
                if(listTemp.length>0){
                    textShow += "共" + listTemp.length + "站次预报低温>高温\n";
                }
                ...
                for(var i=0;i<listTempMin.length;i++){
                    ...
                    textShow += t.stationList[parseInt($(listTempMin[i]).attr("cols"))].StationName + "站";
                    textShow += $(listTempMin[i]).parent().attr("shixiao") + "小时预报:" +$(listTempMin[i]).find("input").val()+"°C\n";
                }
                ...
                for(var i=0;i<listTempMax.length;i++){
                    ...
                    textShow += t.stationList[parseInt($(listTempMax[i]).attr("cols"))].StationName + "站";
                    textShow += $(listTempMax[i]).parent().attr("shixiao") + "小时预报:" +$(listTempMax[i]).find("input").val()+"°C\n";
                }
                ...
                for(var i=0;i<list.length;i++){
                    name = "";
                    if($(list[i]).html().indexOf("霾")!=-1)
                        name = "霾";
                    else if($(list[i]).html().indexOf("扬沙")!=-1)
                        name = "扬沙";
                    else if($(list[i]).html().indexOf("强沙尘暴")!=-1)
                        name = "沙尘";
                    if(name == "")
                        continue;
                    if(!weatherObj.hasOwnProperty(name)){
                ...
                for(var name in weatherObj){
                    ...
                    if(weatherObj[name].length>0){
                        textShow += "共" + weatherObj[name].length + "站次天气预报>=30\n";
                    }
                }
                var listTmin =  $(".ZDYBTable").find("td[type='tmin']");
                var listTmax =  $(".ZDYBTable").find("td[type='tmax']");
                ...
                for(var i=0;i<listTmin.length;i++){
                    if($(listTmin[i]).find("input").val()!=""&&$(listTmax[i]).find("input").val()!=""&&parseFloat($(listTmin[i]).find("input").val())>parseFloat($(listTmax[i]).find("input").val()))
                        listTemp.push(listTmin[i]);
                    if($(listTmin[i]).find("input").val()!=""&&parseFloat($(listTmin[i]).find("input").val())<=-10)
                        listTempMin.push(listTmin[i]);
                    if(parseFloat($(listTmax[i]).find("input").val())>=45)
                        listTempMax.push(listTmax[i]);
                }
                ...
                if(listTempMin.length>0){
                    textShow += "共" + listTempMin.length + "站次预报低于-10°C\n";
                }
                for(var i=0;i<listTempMax.length;i++){
                    if(i==2){
                        textShow += "...\n";
                ...
                if(listTempMax.length>0){
                    textShow += "共" + listTempMax.length + "站次预报高于45°C\n";
                }
            }
        }
        else if(t.nowElement.productName == "乡镇预报"){
            var listTmin =  [];
            var listTmax =  [];
            var listTemp = [];
            var listTempMin = [];
            ...
            var preMax = [];
            var  listTotal = [];
            for(var ele in t.xzData){
                var hourspan = ele.split("_")[1]
                if(ele.indexOf("tmin") != -1){
                    listTmin = t.xzData[ele];
                    listTmax =  [];
                    if(typeof(t.xzData["tmax_"+hourspan]) != "undefined")
                        listTmax = t.xzData["tmax_"+hourspan];
                    else{
                        alert("高温"+ele.split("_")[1]+"无数据");
                        return;
                    }
                    for(var i=0;i<listTmin.length;i++){
                        if(listTmin[i]>listTmax[i]){
                            listTemp.push({station:i,hourSpan:hourspan,value:listTmin[i]});
                            listTotal.push({element:ele+"_"+i,value:t.stationList[i].StationName + "站" + t.stationList[i].StationNum + "\n"+hourspan+ "小时预报：低温>高温"});
                        }
                        if(listTmin[i]<-10){
                            listTempMin.push({element:"低温",station:i,hourSpan:hourspan,value:listTmin[i]});
                            listTotal.push({element:ele+"_"+i,value:t.stationList[i].StationName + "站" + t.stationList[i].StationNum + "\n"+hourspan+ "小时预报：低温低于-10°C"});
                        }
                        if(listTmax[i]<-10){
                            listTempMin.push({element:"高温",station:i,hourSpan:hourspan,value:listTmax[i]});
                            listTotal.push({element:ele+"_"+i,value:t.stationList[i].StationName + "站" + t.stationList[i].StationNum + "\n"+hourspan+ "小时预报：低温低于-10°C"});
                        }
                        if(listTmin[i]>=40){
                            listTempMax.push({element:"低温",station:i,hourSpan:hourspan,value:listTmin[i]});
                            listTotal.push({element:"tmax_"+hourspan+"_"+i,value:t.stationList[i].StationName + "站" + t.stationList[i].StationNum + "\n"+hourspan+ "小时预报：高温高于40°C"});
                        }
                        if(listTmax[i]>=40){
                            listTempMax.push({element:"高温",station:i,hourSpan:hourspan,value:listTmax[i]});
                            listTotal.push({element:"tmax_"+hourspan+"_"+i,value:t.stationList[i].StationName + "站" + t.stationList[i].StationNum + "\n"+hourspan+ "小时预报：高温高于40°C"});
                        }
                    }
                }
                else if(ele.indexOf("r3") != -1 || ele.indexOf("r6") != -1 || ele.indexOf("r12") != -1 || ele.indexOf("r24") != -1){
                    pre = t.xzData[ele];
                    for(var i=0;i<pre.length;i++){
                        if(pre[i] < 0){
                            preMin.push({station:i,hourSpan:hourspan,value:pre[i]});
                            listTotal.push({element:ele+"_"+i,value:t.stationList[i].StationName + "站" + t.stationList[i].StationNum + "\n"+hourspan+ "小时预报：降水小于0mm"});
                        }
                        if(pre[i] > 250){
                            preMax.push({station:i,hourSpan:hourspan,value:pre[i]});
                            listTotal.push({element:ele+"_"+i,value:t.stationList[i].StationName + "站" + t.stationList[i].StationNum + "\n"+hourspan+ "小时预报：降水大于250mm"});
                        }
                    }
                }
            }
            for(var i=0;i<listTemp.length;i++){
                ...
                textShow += t.stationList[listTemp[i].station].xAreaName+" "+t.stationList[listTemp[i].station].StationName + "站" + t.stationList[listTemp[i].station].StationNum + "/";
                textShow += listTemp[i].hourSpan + "小时预报:" +"低温>高温\n";
            }
            if(listTemp.length>0){
                textShow += "共" + listTemp.length + "站次预报低温>高温\n";
            }
            for(var i=0;i<listTempMin.length;i++){
                ...
                textShow += t.stationList[listTempMin[i].station].xAreaName+" "+t.stationList[listTempMin[i].station].StationName + "站" + t.stationList[listTempMin[i].station].StationNum + "/";
                textShow += listTempMin[i].hourSpan + "小时预报"+listTempMin[i].element+":" + listTempMin[i].value +"°C\n";
            }
            if(listTempMin.length>0){
                textShow += "共" + listTempMin.length + "站次预报低于-10°C\n";
            }
            for(var i=0;i<listTempMax.length;i++){
                ...
                textShow += t.stationList[listTempMax[i].station].xAreaName+" "+t.stationList[listTempMax[i].station].StationName + "站" + t.stationList[listTempMax[i].station].StationNum + "/";
                textShow += listTempMax[i].hourSpan + "小时预报"+listTempMax[i].element+":" + listTempMax[i].value +"°C\n";
            }
            if(listTempMax.length>0){
                textShow += "共" + listTempMax.length + "站次预报高于40°C\n";
            }

            for(var i=0;i<preMin.length;i++){
                if(i==10){
                    textShow += "...\n";
                    break;
                }
                textShow += t.stationList[preMin[i].station].xAreaName+" "+t.stationList[preMin[i].station].StationName + "站" + t.stationList[preMin[i].station].StationNum + "/";
                textShow += preMin[i].hourSpan + "小时预报降水:" + preMin[i].value +"mm\n";
            }
            if(preMin.length>0){
                textShow += "共" + preMin.length + "站次预报小于0mm\n";
            }

            for(var i=0;i<preMax.length;i++){
                if(i==10){
                    textShow += "...\n";
                    break;
                }
                textShow += t.stationList[preMax[i].station].xAreaName+" "+t.stationList[preMax[i].station].StationName + "站" + t.stationList[preMax[i].station].StationNum + "/";
                textShow += preMax[i].hourSpan + "小时预报降水:" + preMax[i].value +"mm\n";
            }
            if(preMax.length>0){
                textShow += "共" + preMax.length + "站次预报大于250mm\n";
            }
        }
        if(elementName)
            return {textShow:textShow,list:listTotal};
        else
            return textShow;
    }
